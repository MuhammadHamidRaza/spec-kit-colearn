...
# ///
"""
Spec-Kit-CoLearn CLI - AI Co-Learning Spec Framework

Build with AI as your senior architect co-partner. Learn while you build.

Usage:
    uvx spec-kit-colearn.py init <project-name>
    uvx spec-kit-colearn.py init .
    uvx spec-kit-colearn.py init --here

Or install globally:
    uv tool install --from spec-kit-colearn.py spec-kit-colearn
    spec-kit-colearn init <project-name>
...
def get_version() -> str:
    """Get the current version of spec-kit-colearn."""
    import importlib.metadata

    try:
        # Try spec-kit-colearn first (PyPI package name)
...
def version_callback(value: bool):
    ...
    if value:
        ...
        console.print(f"Spec-Kit-CoLearn CLI v{pkg_version}")
        raise typer.Exit()
...
def download_template_from_github(
    ...
    *,
    ...
    repo_owner = "muhammadhamidraza"
    repo_name = "spec-kit-colearn"
    if client is None:
        client = httpx.Client(verify=ssl_context)
...
def version():
    ...
    # Fetch latest template release version from spec-kit-colearn
    repo_owner = "muhammadhamidraza"
    repo_name = "spec-kit-colearn"
    api_url = f"https://api.github.com/repos/{repo_owner}/{repo_name}/releases/latest"
    ...
    panel = Panel(
        info_table,
        title="[bold cyan]Spec-Kit-CoLearn CLI Information[/bold cyan]",
        border_style="cyan",
        padding=(1, 2),
    )
